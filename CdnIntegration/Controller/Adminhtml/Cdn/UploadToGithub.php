...
class UploadToGithub extends Action
{
    ...
    public function execute()
    {
     ...
     // كود محسن لتحديد المسار المحلي مع دعم إضافي للخطوط
$localPath = '';
$remotePath = '';
...
if (strpos($url, '/static/') === 0) {
    ...
    // تعامل خاص مع ملفات الخطوط التي قد تكون في مسارات مختلفة
    if (strpos($url, '.woff') !== false || strpos($url, '.woff2') !== false || 
        strpos($url, '.ttf') !== false || strpos($url, '.eot') !== false) {
        
        $this->helper->log("Font file detected: {$url}", 'info');
        
        // في حالة عدم وجود الملف في المسار المتوقع، البحث في مكان آخر
        if (!file_exists($localPath)) {
            // تجربة البحث في مسارات أخرى للخطوط
            $fontPaths = [
                $staticDir . 'frontend/' . dirname(dirname(dirname($path))) . '/fonts/' . basename($path),
                $staticDir . dirname(dirname($path)) . '/fonts/' . basename($path),
            ];
            
            foreach ($fontPaths as $fontPath) {
                if (file_exists($fontPath)) {
                    $this->helper->log("Font found at alternative path: {$fontPath}", 'info');
                    $localPath = $fontPath;
                    break;
                }
            }
